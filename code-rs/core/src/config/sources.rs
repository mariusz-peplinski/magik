...
/// Persist the Auto Review footer display mode into `CODEX_HOME/config.toml` at
/// `[tui].auto_review_symbols_only`.
pub fn set_tui_auto_review_symbols_only(code_home: &Path, enabled: bool) -> anyhow::Result<()> {
    let config_path = code_home.join(CONFIG_TOML_FILE);
    let read_path = resolve_code_path_for_read(code_home, Path::new(CONFIG_TOML_FILE));
    ...
    doc["tui"]["auto_review_symbols_only"] = toml_edit::value(enabled);
...
/// Persist the Git snapshot preference into `CODEX_HOME/config.toml` at `[tui].git_snapshots_enabled`.
pub fn set_tui_git_snapshots_enabled(code_home: &Path, enabled: bool) -> anyhow::Result<()> {
    let config_path = code_home.join(CONFIG_TOML_FILE);
    let read_path = resolve_code_path_for_read(code_home, Path::new(CONFIG_TOML_FILE));
    ...
    doc["tui"]["git_snapshots_enabled"] = toml_edit::value(enabled);
...
/// Persist the reasoning visibility preference into `CODEX_HOME/config.toml` at `[tui].show_reasoning`.
pub fn set_tui_show_reasoning(code_home: &Path, enabled: bool) -> anyhow::Result<()> {
    let config_path = code_home.join(CONFIG_TOML_FILE);
    let read_path = resolve_code_path_for_read(code_home, Path::new(CONFIG_TOML_FILE));
    ...
    doc["tui"]["show_reasoning"] = toml_edit::value(enabled);
...
pub fn set_tui_show_block_type_labels(code_home: &Path, enabled: bool) -> anyhow::Result<()> {
    ...
    doc["tui"]["show_block_type_labels"] = toml_edit::value(enabled);
...
/// Persist whether to render rounded corners on bordered boxes into `CODEX_HOME/config.toml`.
pub fn set_tui_rounded_corners(code_home: &Path, enabled: bool) -> anyhow::Result<()> {
    let config_path = code_home.join(CONFIG_TOML_FILE);
    let read_path = resolve_code_path_for_read(code_home, Path::new(CONFIG_TOML_FILE));
    ...
    doc["tui"]["rounded_corners"] = toml_edit::value(enabled);
