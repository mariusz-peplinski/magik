    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.is_task_running && !self.embedded_mode {
            if self.auto_drive_active {
                ...
                let interval_ms = def.interval_ms.max(50);
                ...
                // Ensure time-based spinner frames keep advancing even when the
                // model is idle (no streaming output) and the user isn't typing.
                self.app_event_tx
                    .send(AppEvent::ScheduleFrameIn(Duration::from_millis(interval_ms)));
            }
        }
